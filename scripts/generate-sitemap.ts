...
const urls = [
  ...
  ...CASE_STUDIES.map((study) => ({ path: `/portfolio/${study.id}`, lastmod: today, priority: '0.8' })),
  ...BLOG_POSTS.map((post) => ({ path: `/blog/${post.slug}`, lastmod: post.lastUpdated, priority: '0.8' })),
];
