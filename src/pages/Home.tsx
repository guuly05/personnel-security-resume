...
const lanes = [
  ...
  { label: 'Delivery', detail: 'Pipelines that keep shipping', icon: 'git-branch', tone: 'text-[var(--brand-purple)]' },
];
  ...
  <div className="space-y-5">
      ...
      <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div>
          ...
          <h1 className="font-display text-5xl font-bold leading-[0.98] tracking-[-0.05em] text-[var(--color-text)] md:text-7xl">
            Build the product.
            <span className="block text-gradient">Ship the system.</span>
          </h1>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <a href="/portfolio" className="inline-flex items-center gap-2 rounded-full bg-[var(--accent)] px-5 py-3 text-sm font-bold text-[var(--color-bg)] shadow-[0_12px_30px_color-mix(in_srgb,var(--accent)_22%,transparent)] transition hover:-translate-y-0.5">
        ...
        <div className="relative">
          <div className="home-stack-card rounded-[1.6rem] border border-[var(--border)] bg-[var(--surface-soft)] p-4 shadow-2xl sm:p-5">
            ...
            <div className="space-y-3">
              {lanes.map((lane, index) => (
                <React.Fragment key={lane.label}>
                  <div className="flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3.5">
          ...
          <div className="pointer-events-none absolute -bottom-5 -right-3 -z-10 h-32 w-32 rounded-full bg-[var(--brand-purple)]/20 blur-3xl" />
        </div>
      </div>
    </section>
    ...
    <section className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
      <div className="surface-card p-6 md:p-8">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">{ACHIEVEMENTS.map((achievement) => <div key={achievement.text} className="rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-4"><p className="font-mono text-xs font-bold text-[var(--accent)]">{achievement.metric}</p><p className="mt-2 text-sm leading-relaxed text-[var(--color-text-muted)]">{achievement.text}</p></div>)}</div>
      </div>
      <div className="surface-card p-6 md:p-8">
        <p className="eyebrow text-[var(--brand-purple)]">Current toolkit</p>
        <div className="mt-3 flex items-end justify-between gap-4"><h2 className="font-display text-2xl font-bold">A broad base, always getting deeper.</h2><span className="font-mono text-4xl font-bold text-[var(--brand-purple)]">{CORE_SKILLS.length}</span></div>
